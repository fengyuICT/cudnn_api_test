...
void print(const float *data, int n, int c, int h, int w) {
  ...
  float const_answer[42] = {
        3,  15,  33,  51,  69,  87,  48,
       60, 138, 174, 210, 246, 282, 150,
      168, 354, 390, 426, 462, 498, 258,
      276, 570, 606, 642, 678, 714, 366,
      384, 786, 822, 858, 894, 930, 474,
      219, 447, 465, 483, 501, 519, 264
  };
  int a = 0;
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < c; ++j) {
      ...
      for (int k = 0; k < h; ++k) {
        for (int l = 0; l < w; ++l) {
          ...
          if (const_answer[a] != buffer[a]) {
            std::cout << "Occurs some error\n";
          }
          ++a;
        }
        std::cout << std::endl;
...
int main() {
  ...
  int convDim = 2;
  int dimA[4] = {in_n, in_c, in_h, in_w};
  int strideA[4] = {90, 1, 18, 3};
  CUDNN_CALL(cudnnSetTensorNdDescriptor(in_desc, CUDNN_DATA_FLOAT, convDim+2, dimA, strideA));
